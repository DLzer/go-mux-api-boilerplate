...
func (a *App) Initialize(user, password, dbname string) {
	...
	// fmt.Sprintf("user=%s password=%s dbname=%s sslmode=disable", "postgres", "postgres", "postgres")
...
// Run will start the HTTP Server for listening on 8010
...
// respondWithError will normalize error resoponses to JSON format
func respondWithError(w http.ResponseWriter, code int, message string) {
	respondWithJSON(w, code, map[string]string{"error": message})
}

// respondWithJSON will normalize all API response to JSON format
